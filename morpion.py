...
def menu(): #choice beetween replay or go start
	window.blit(pygame.image.load("images/white.png").convert(), (580, 542)) #remove the next player
	pygame.display.flip()
	time.sleep(1.5)
	window.blit(pygame.image.load("images/menu.png").convert(), (230, 200))
	pygame.display.flip()
	while True:
		for event in pygame.event.get():
			if event.type == QUIT:
...
def morpion():
	...
	if player == 1:
		window.blit(pygame.image.load("images/tdagger.png").convert(), (580, 542))
	else:
		window.blit(pygame.image.load("images/tround.png").convert(), (580, 542))
	pygame.display.flip()
	n = 0
	while True:
		case = selectcase()
		if background[case] == 0: #if the case don't have pion
			...
			if player == 1: #display and change the player
				window.blit(pygame.image.load("images/dagger.png").convert(), case)
				player=2
				window.blit(pygame.image.load("images/tround.png").convert(), (580, 542))
			else: #display and change the player
				window.blit(pygame.image.load("images/round.png").convert(), case)
				player=1
				window.blit(pygame.image.load("images/tdagger.png").convert(), (580, 542))
			pygame.display.flip()
			if n > 4: #before we can't have a winner
				if w == 1:
					window.blit(pygame.image.load("images/gdagger.png").convert(), (200, 542))
					menu()
				if w == 2:
					window.blit(pygame.image.load("images/ground.png").convert(), (200, 542))
					menu()
				if w == 3:
					window.blit(pygame.image.load("images/equality.png").convert(), (230, 542))
					menu()
